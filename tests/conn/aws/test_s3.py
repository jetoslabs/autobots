...
@pytest.mark.asyncio
async def test_aws_s3_happy_path(set_settings):
    ...
    # put data in s3
    res1 = await get_conn().s3.put(data=str1, filename=gen_filename(str1))
    assert res1 == len(str1)

    # get data from file
    res1_data = await get_conn().s3.get(filename=gen_filename(str1))
