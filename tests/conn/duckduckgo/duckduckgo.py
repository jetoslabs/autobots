...
from autobots.conn.duckduckgo.duckduckgo import SearchRes
...
async def test_answer_happy_path(set_settings):
    ans_res = await get_conn().duckduckgo.answer("sun")
...
async def test_search_images_happy_path(set_settings):
    ans_res = await get_conn().duckduckgo.search_images("Athlete running in the city")
    assert len(ans_res) > 0
    assert ans_res[0].url
    assert ans_res[0].title
...
async def test_search_videos_happy_path(set_settings):
    ans_res = await get_conn().duckduckgo.search_videos("Athlete running in the city")
    assert len(ans_res) > 0
...
async def test_search_map_happy_path(set_settings):
    ans_res = await get_conn().duckduckgo.search_map("monuments", city="New Delhi")
    assert len(ans_res) > 0
    assert ans_res[0].url
    assert ans_res[0].title
...
async def test_translate_happy_path(set_settings):
    ans_res = await get_conn().duckduckgo.translate("Going to school", to="hi")
...
async def test_suggestions_happy_path(set_settings):
    ans_res = await get_conn().duckduckgo.suggestions("arsenal.com")
    assert len(ans_res) > 0
