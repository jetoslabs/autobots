...
class UserActionGraphs:
    ...
    async def create(
        ...
        action_graph_doc_create = ActionGraphDocCreate(
            user_id=self.user_id, **action_graph_create.model_dump()
        )
    ...
    async def list(
        self,
        action_graph_find: ActionGraphFind,
        db: Database = Depends(get_mongo_db),
        limit: int = 100,
        offset: int = 0,
    ) -> List[ActionGraphDoc]:
        action_graph_doc_find = ActionGraphDocFind(
            user_id=self.user_id, **action_graph_find.model_dump()
        )
        action_graph_docs = await ActionGraphCRUD(db).find(
            action_graph_doc_find, limit, offset
        )
    ...
    async def get(
        ...
        action_graph_doc_find = ActionGraphDocFind(
            id=action_graph_id, user_id=self.user_id
        )
    ...
    async def update(
        ...
        action_graph_doc_update = ActionGraphDocUpdate(
            id=action_graph_id, user_id=self.user_id, **action_graph_update.model_dump()
        )
