...
router = APIRouter(
    prefix=SettingsProvider.sget().API_ACTION_GRAPHS_RESULTS,
...
async def list_action_graph_result(
    ...
    action_result_docs = await user_action_graph_result.list_action_graph_result(
        action_result_find, limit, offset
    )
...
async def get_action_result(
    ...
    user_orm = UserORM(id=UUID(user_res.user.id))
    action_result_doc = await UserActionGraphResult(
        user_orm, db
    ).get_action_graph_result(id)
    return action_result_doc
