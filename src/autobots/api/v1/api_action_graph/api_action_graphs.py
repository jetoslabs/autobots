...
router = APIRouter(
    prefix=SettingsProvider.sget().API_ACTION_GRAPHS,
    tags=[SettingsProvider.sget().API_ACTION_GRAPHS],
)
...
async def create_action_graph(
    ...
    try:
        ...
        resp = await UserActionGraphs(user=user_orm, db=db).create(
            action_graph_create, db
        )
        return resp
...
async def list_action_graphs(
    ...
    action_docs = await UserActionGraphs(user=user_orm, db=db).list(
        find, db, limit, offset
    )
    return action_docs
...
async def update_action_graph(
    ...
    user_orm = UserORM(id=UUID(user_res.user.id))
    action_doc = await UserActionGraphs(user=user_orm, db=db).update(
        id, action_graph_update, db
    )
    return action_doc
