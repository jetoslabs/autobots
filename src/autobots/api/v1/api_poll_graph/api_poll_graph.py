...
from src.autobots.poll_graph.poll_graph_common_models import TextObj
...
from src.autobots.poll_graph.poll_graph_result.poll_graph_result_model_doc import PollGraphResultDoc
from src.autobots.poll_graph.poll_graph_result.user_poll_graph_result import UserPollGraphResult
...
from src.autobots.poll_graph.poll_graph.user_poll_graph import UserPollGraphs
...
async def create_poll_graph(
    ...
    try:
        ...
        resp = await UserPollGraphs(user=user_orm, db=db).create(poll_graph_create)
        return resp
...
async def list_poll_graphs(
    ...
    poll_graph_docs = await UserPollGraphs(user=user_orm, db=db).list_owned_or_published(find, or_find, limit, offset)
...
async def get_poll_graph(
    ...
    user_orm = UserORM(id=UUID(user_res.user.id))
    poll_graph_doc = await UserPollGraphs(user=user_orm, db=db).get_owned_or_published(id)
...
async def update_poll_graph(
    ...
    user_orm = UserORM(id=UUID(user_res.user.id))
    poll_graph_doc = await UserPollGraphs(user=user_orm, db=db).update(id, poll_graph_update)
    return poll_graph_doc
...
async def delete_poll_graph(
    ...
    user_orm = UserORM(id=UUID(user_res.user.id))
    user_poll_graphs = UserPollGraphs(user=user_orm, db=db)
...
# ) -> Dict[str, Any]:
#     user_orm = UserORM(id=UUID(user_res.user.id))
#     resp = await UserPollGraphs(user=user_orm, db=db).run(id, input, db)
#     return resp
...
async def async_run_poll_graph(
    ...
    resp = await UserPollGraphs(user=user_orm, db=db).run_in_background(
        ctx,
