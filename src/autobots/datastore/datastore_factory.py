...
from src.autobots.conn.unstructured_io.unstructured_io import get_unstructured_io, UnstructuredIO
...
class DatastoreFactory:
    ...
    async def create_datastore(self, name: str) -> Datastore:
        datastore = Datastore(s3=self.s3, pinecone=self.pinecone, unstructured=self.unstructured).init(name=name)
        return datastore

    async def hydrate_datastore(self, datastore_id: str) -> Datastore:
        datastore = Datastore(s3=self.s3, pinecone=self.pinecone, unstructured=self.unstructured).hydrate(
...
def get_datastore_factory(settings: Settings = SettingsProvider.sget()) -> DatastoreFactory:
    return DatastoreFactory(s3=get_s3(), pinecone=get_pinecone(), unstructured=get_unstructured_io())
