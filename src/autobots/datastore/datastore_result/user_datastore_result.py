...
class UserDatastoreResult:
    ...
    async def create_datastore_result(
        self, datastore_result: DatastoreResultCreate
    ) -> DatastoreResultDoc | None:
        try:
            event_result_doc_create = EventResultDocCreate(
                user_id=self.user_id, **datastore_result.model_dump()
            )
            event_result_doc = await self.event_result_crud.insert_one(
                event_result_doc_create
            )
            datastore_result_doc = DatastoreResultDoc.model_validate(
    ...
    async def list_datastore_result(
        self, datastore_result_find: EventResultFind, limit: int = 100, offset: int = 0
    ) -> List[DatastoreResultDoc]:
        event_result_doc_find = EventResultDocFind(
            user_id=self.user_id, **datastore_result_find.model_dump()
        )
    ...
    async def get_datastore_result(
        ...
        try:
            event_result_doc_find = EventResultDocFind(
                id=datastore_result_id, user_id=self.user_id
            )
    ...
    async def update_datastore_result(
        ...
        try:
            event_result_doc_update = EventResultDocUpdate(
                id=datastore_result_id,
                user_id=self.user_id,
                **datastore_update.model_dump(),
            )
    ...
    async def delete_datastore_result(self, id: str) -> int:
