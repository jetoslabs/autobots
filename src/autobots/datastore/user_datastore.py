...
from src.autobots.datastore.datastore_result.datastore_result_doc_model import (
    DatastoreResultCreate,
    DatastoreResultDoc,
    DatastoreResultUpdate,
)
from src.autobots.datastore.datastore_result.user_datastore_result import (
    UserDatastoreResult,
)
...
class UserDatastore:
    ...
    async def init(self, name: str) -> DatastoreMetaDoc:
        ...
        datastore_meta_doc_create = DatastoreMetaDocCreate(
            user_id=str(self.user.id),
            datastore_id=self.datastore.id,
            name=self.datastore.name,
        )
        user_datastore_meta = await self.datastore_meta_crud.insert_one(
            datastore_meta_doc_create
        )
        return user_datastore_meta

    async def hydrate(self, datastore_id: str) -> Datastore:
        datastore_meta_doc_find = DatastoreMetaDocFind(
            user_id=str(self.user.id), datastore_id=datastore_id
        )
    ...
    async def get(self, name: str) -> DatastoreMetaDoc:
        datastore_meta_doc_find = DatastoreMetaDocFind(
            user_id=str(self.user.id), name=name
        )
        datastore_metas = await self.datastore_meta_crud.find(datastore_meta_doc_find)
        if len(datastore_metas) != 1:
    ...
    async def list(self, limit: int = 100, offset: int = 0) -> List[DatastoreMetaDoc]:
        ...
        datastore_metas = await self.datastore_meta_crud.find(
            datastore_meta_doc_find, limit, offset
        )
    ...
    async def delete(self, datastore_id: str) -> DeleteResult:
        ...
        deleted_result = await self.datastore_meta_crud.delete_many(
            datastore_meta_doc_find
        )
    ...
    async def put_resource_async(
        ...
        if user_datastore is None:
            raise HTTPException(404, "Datastore not found")
        datastore_result_create = DatastoreResultCreate(
            status=EventResultStatus.processing.value
        )
        datastore_result_doc = await self.user_datastore_result.create_datastore_result(
            datastore_result_create
        )
    ...
    async def put_resource(
        ...
        if data:
            try:
                datastore_results_1 = await user_datastore.put_data(
                    data, chunk_token_size=chunk_size
                )
                datastore_result_doc.result.status_for = (
                    datastore_result_doc.result.status_for + datastore_results_1
                )
                datastore_result_doc = (
                    await user_datastore_result.update_datastore_result(
                        datastore_result_doc.id,
                        DatastoreResultUpdate(**datastore_result_doc.model_dump()),
                    )
                )
        ...
        if files:
            try:
                ...
                datastore_result_doc.result.status_for = (
                    datastore_result_doc.result.status_for + datastore_results_2
                )
                datastore_result_doc = (
                    await user_datastore_result.update_datastore_result(
                        datastore_result_doc.id,
        ...
        if urls:
            try:
                datastore_results_3 = await user_datastore.put_urls(
                    urls, chunk_token_size=chunk_size
                )
                datastore_result_doc.result.status_for = (
                    datastore_result_doc.result.status_for + datastore_results_3
                )
                datastore_result_doc = (
                    await user_datastore_result.update_datastore_result(
                        datastore_result_doc.id,
    ...
    async def get_datastore_put_result(
        self, datastore_put_result_id: str
    ) -> DatastoreResultDoc | None:
        return await self.user_datastore_result.get_datastore_result(
            datastore_put_result_id
        )
