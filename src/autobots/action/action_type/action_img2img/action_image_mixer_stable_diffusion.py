...
class ImageMixerRunModel(BaseModel):
    ...
    init_image: Optional[str] = Field(
        None, description="comma separated image urls of images to mix"
    )
    init_image_weights: Optional[str] = Field(
        None,
...
class ActionImageMixerStableDiffusion(
    IAction[ImageMixerReqModel, ImageMixerRunModel, StableDiffusionRes]
):
    ...
    async def run_action(self, action_input: ImageMixerRunModel) -> StableDiffusionRes:
        ...
        if action_input.init_image:
            self.action_config.init_image = (
                f"{self.action_config.init_image},{action_input.init_image}"
            )
