...
class UserActionResult:
    ...
    async def list_action_result(
        self, action_result_find: EventResultFind, limit: int = 100, offset: int = 0
    ) -> List[ActionResultDoc]:
        action_result_find.type = EventType.action_run
        event_result_doc_find = EventResultDocFind(
            user_id=self.user_id, **action_result_find.model_dump(exclude_none=True)
        )
    ...
    async def get_action_result(self, action_result_id: str) -> ActionResultDoc | None:
        try:
            event_result_doc_find = EventResultDocFind(
                id=action_result_id, user_id=self.user_id
            )
    ...
    async def delete_action_result(self, id: str) -> int:
