...
class ReasonActObserve:
    def __init__(self):
        base_prompt = (
            "Follow cycle of thought, act and/or observe until you finish the task"
        )
        ...
        self.setup_messages: List[ChatCompletionMessageParam] = [
            ...
            ChatCompletionAssistantMessageParam(
                role="assistant", content=thought_goal_example_1
            ),
            ChatCompletionAssistantMessageParam(
                role="assistant", content=action_example_1
            ),
            ChatCompletionSystemMessageParam(
                role="system", content=observation_example_1
            ),
            ChatCompletionAssistantMessageParam(
                role="assistant", content=thought_goal_example_2
            ),
            ChatCompletionAssistantMessageParam(
                role="assistant", content=action_example_2
            ),
        ]
    ...
    async def do_task(self, user_goal: str) -> List[ChatCompletionMessageParam]:
        messages = self.setup_messages + [
            ChatCompletionUserMessageParam(
                role="user", content=f"{Task_Prefix}{user_goal}"
            )
        ]
        ...
        while not is_finish:
            ...
            messages = messages + [
                ChatCompletionAssistantMessageParam(role="assistant", content=thought)
            ]

            action = await self.act(messages)
            messages = messages + [
                ChatCompletionAssistantMessageParam(role="assistant", content=action)
            ]

            observation = await self.observe(action=action)
            if observation:
                messages = messages + [
                    ChatCompletionSystemMessageParam(role="system", content=observation)
                ]
    ...
    async def think(self, messages: List[ChatCompletionMessageParam]) -> str:
        req_message = messages + [
            ChatCompletionUserMessageParam(
                role="user", content="Now Think. Respond in maximum of 500 words"
            )
        ]
        chat_req: ChatReq = ChatReq(
            messages=req_message, max_tokens=500, temperature=0.8
        )
    ...
    async def act(self, messages: List[ChatCompletionMessageParam]) -> str:
        try:
            req_message = messages + [
                ChatCompletionUserMessageParam(
                    role="user",
                    content="Based on above thought, Now Select one Action and one action only",
                )
            ]
            chat_req: ChatReq = ChatReq(
                messages=req_message, max_tokens=500, temperature=0.8
    ...
    async def observe(self, action: str) -> str:
        if "search" in action:
            ...
            for search in search_res:
                res = (
                    res
                    + f"Title:{search.title}, Excerpt: {search.body}, URL: {search.href}\n"
                )
