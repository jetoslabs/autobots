...
class OneStepAgent:
    async def run(self, agent_data: AgentData, loops_allowed=5):
        agent_data.context.append(
            ChatCompletionUserMessageParam(role="user", content=f"{agent_data.goal}")
        )
    ...
    async def generate_prompt_for_goal(self, agent_data) -> str:
        msg1 = ChatCompletionUserMessageParam(
            role="user", content=f"My goal: {agent_data.context[-1]}"
        )
        chat_req = ChatReq(
            messages=prompt_generator_messages + [msg1]
        )  # + agent_data.context)
    ...
    async def run_next_action(self, next_action_str, agent_data: AgentData):
        ...
        if next_action == "LLMChat":
            chat_req: ChatReq = ChatReq(
                messages=[
                    ChatCompletionUserMessageParam(
                        role="user", content=next_action_input
                    )
                ]
            )
            ...
            agent_data.context.append(
                ...
                )  # content=llm_chat_data.context[-1].content)
            )
    ...
    async def plan_for_goal(self, agent_data):
        msg1 = ChatCompletionUserMessageParam(
            role="user", content=f"My goal: {agent_data.goal}"
        )
