...
class Auth:
    ...
    async def sign_in_with_password(
        ...
        res: gotrue.AuthResponse = self.supabase_client.auth.sign_in_with_password(
    ...
    async def sign_up_with_password(
        ...
        if not res.user:
            raise HTTPException(
                status_code=409, detail="System conflict while creating user"
            )
        return res

    async def reset_password_email(
        self, email: EmailStr, redirect_to: HttpUrl = None
    ) -> bool:
        options = {}
        if redirect_to:
    ...
    async def refresh_session(self, refresh_token: str) -> gotrue.AuthResponse:
        res: gotrue.AuthResponse = self.supabase_client.auth.refresh_session(
